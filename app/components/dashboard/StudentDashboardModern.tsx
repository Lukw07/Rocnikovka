...
import { ItemManagement } from "@/app/components/shop/ItemManagement"
...
export function StudentDashboardModern({ userId, classId }: StudentDashboardProps) {
  ...
  // If sidebar selected a different panel, render it here
  if (selectedPanel === 'shop') {
    return <ShopInterface userId={userId} userRole={UserRole.STUDENT} />
  }

  // Render specific panel components
  if (selectedPanel === 'subjects') return <SubjectsPanel />
  if (selectedPanel === 'achievements') return <AchievementsPanel />
  if (selectedPanel === 'leaderboard') return <LeaderboardPanel />
