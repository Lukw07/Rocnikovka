...
const PII_FIELDS = [
  ...
  // Note: 'user' is not included as it can refer to user objects that should be processed recursively
  // Note: 'count' is not included as it's a safe numeric field
] as const
...
export function redactPIIFromObject(obj: any): any {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    // Check if this field should be redacted - exact match only
...
export function createSafeLogMetadata(data: Record<string, any>): SafeLogMetadata {
  ...
  // Extract only safe fields - userId is allowed in logs as it's an ID, not PII
  const safe: SafeLogMetadata = {}
  
  if (redacted.requestId) safe.requestId = redacted.requestId
  if (redacted.userId && redacted.userId !== '[REDACTED]') safe.userId = redacted.userId
